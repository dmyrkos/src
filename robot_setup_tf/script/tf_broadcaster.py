...
def update_dic(servo_data):
    ...
    if s[-1] == 'H' :
        dynamic_links[s1][s2].insert(6,dynamic_links[s1][s2][5] + angle )
    elif s[-1] == 'K' :
        dynamic_links[s1][s2].insert(5, dynamic_links[s1][s2][6]  + angle)
    
    global counter
    counter = counter + 1


    rospy.loginfo( "name : %s , angle : %s  x : %d ,y : %d , z :%d",s1 ,s2,angle,dynamic_links[s1][s2][5],dynamic_links[s1][s2][6])
    if counter >= 12 :
        dynamic_links = init_pos.copy()
        counter = 0
        print("----------------------------------------------------------")
        print(dynamic_links)
    servo_rot_callback()
